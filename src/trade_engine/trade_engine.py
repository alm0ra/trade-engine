...
class TradeEngine:
    ...
    def set_initial_balance(self, balance: float):
        ...
        self.initial_balance = Balance(balance=balance).balance

    def set_commision(self, commision: float):
        """set commision for demo trade

        Args:
            commision (float): a positive float number < 1 that show commision
        """

        self.commision = Commision(commision=commision).commision
        ...
        for position in self.open_positions:
            ...
            if side == "long":
                ...
                if low <= sl <= high or low < sl:
                    ...
                    ] = self.operations.realized_pnl_percent(
                        side=side, entry_price=enter_price, close_price=close
                    )
            if side == "short":
                # check unrealized profit status
    ...
    def place_market_order(
        ...
        position = {
            ...
            "order_type": "market",
            "side": new_order.side,
            "enter_price": new_order.entry_price,
            "stop_loss": new_order.sl,
            "take_profit": new_order.tp,
    ...
    def calculate_account_loss_profit(self):
        ...
        for trade in self.trade_list:
            self.final_balance = self.initial_balance + round(
                float(trade["realized_profit_loss"]), 2
            )
    ...
    def close_position(self, position, closed_price):
        ...
        if side == "long":
            position["realized_profit_loss"] = self.operations.realized_pnl(
                side="long",
                volume=volume,
                entry_price=entry_price,
            ...
            position[
                ...
                side="long", entry_price=entry_price, close_price=closed_price
            )
            position["final_volume"] = self.operations.final_volume(
                side="long",
                volume=volume,
                entry_price=entry_price,
                close_price=closed_price,
        ...
        if side == "short":
            ...
            position[
                ...
                side="short", entry_price=entry_price, close_price=closed_price
            )
            position["final_volume"] = self.operations.final_volume(
                side="short",
                volume=volume,
